...
def test_create_attendees_valid_date(random_data_create):
    ...
    status, data = create_attendees(sent={"attendees": valid_date})
    assert status == 201
    assert len(data["data"]["successfull"]["mispar_ishi"]) == 1

def test_create_attendees_arrived_as_true(random_data_create):
    mispar_ishi, tehudat_zehut = random_data_create
    arrived_true = [{"mispar_ishi": mispar_ishi, "tehudat_zehut": tehudat_zehut, "full_name": "John Doe", "arrived": True}]
    status, data = create_attendees(sent={"attendees": arrived_true})
    assert status == 201
    assert len(data["data"]["successfull"]["mispar_ishi"]) == 1

def test_create_attendees_arrived_as_false(random_data_create):
    mispar_ishi, tehudat_zehut = random_data_create
    arrived_false = [{"mispar_ishi": mispar_ishi, "tehudat_zehut": tehudat_zehut, "full_name": "John Doe", "arrived": False}]
    status, data = create_attendees(sent={"attendees": arrived_false})
    assert status == 201
    assert len(data["data"]["successfull"]["mispar_ishi"]) == 1

def test_create_attendees_invalid_arrived(random_data_create):
    mispar_ishi, tehudat_zehut = random_data_create
...
def test_create_attendees_invalid_combined_fields():
    invalid_combined = [{
        "mispar_ishi": "123",  
