...
async def test_edit_attendees_missing_id():
    status, data = await edit_attendees(sent={"full_name": "Updated Name"})
    assert status == 400
    assert data["error_code"] == 102

@pytest.mark.asyncio
async def test_edit_attendees_no_fields_to_update():
    status, data = await edit_attendees(sent={"id": 1})
    assert status == 400
    assert data["error_code"] == 101

@pytest.mark.asyncio
async def test_edit_attendees_invalid_id():
    status, data = await edit_attendees(sent={"id": "invalid", "full_name": "Updated Name"})
    assert status == 400
    assert data["error_code"] == 102
...
async def test_edit_attendees_valid_id_update_name():
    ...
    status, _ = await edit_attendees(sent={"id": id_to_edit, "full_name": "Updated Name"})
    assert status == 200

@pytest.mark.asyncio
async def test_edit_attendees_invalid_mispar_ishi(random_data_edit):
    random_id, _, _ = random_data_edit
    status, data = await edit_attendees(sent={"id": random_id, "mispar_ishi": "123"})
    assert status == 400
...
async def test_edit_attendees_invalid_tehudat_zehut(random_data_edit):
    random_id, _, _ = random_data_edit
    status, data = await edit_attendees(sent={"id": random_id, "tehudat_zehut": "12345"})
    assert status == 400
...
async def test_edit_attendees_valid_id_update_multiple_fields(random_data_edit):
    ...
    status, data = await edit_attendees(
        sent={
            "id": id_to_edit,
...
async def test_edit_attendees_invalid_arrived():
    id_to_edit = await create_get_id_attendee()
    status, data = await edit_attendees(sent={"id": id_to_edit, "arrived": "yes"})
    assert status == 400
...
async def test_edit_attendees_valid_arrived_true():
    id_to_edit = await create_get_id_attendee()
    status, data = await edit_attendees(sent={"id": id_to_edit, "arrived": True})
    assert status == 200

@pytest.mark.asyncio
async def test_edit_attendees_valid_arrived_false():
    id_to_edit = await create_get_id_attendee()
    status, data = await edit_attendees(sent={"id": id_to_edit, "arrived": False})
    assert status == 200
...
async def test_edit_attendees_invalid_date_format():
    id_to_edit = await create_get_id_attendee()
    status, data = await edit_attendees(sent={"id": id_to_edit, "date_arrived": "2023-13-32"})
    assert status == 400
...
async def test_edit_attendees_invalid_date_type():
    id_to_edit = await create_get_id_attendee()
    status, data = await edit_attendees(sent={"id": id_to_edit, "date_arrived": 123456789})
    assert status == 400
    assert data["error_code"] == 5
...
async def test_edit_attendees_null_date():
    id_to_edit = await create_get_id_attendee()
    status, data = await edit_attendees(sent={"id": id_to_edit, "date_arrived": None})
    assert status == 400
    assert data["error_code"] == 5
...
async def test_edit_attendees_valid_date():
    id_to_edit = await create_get_id_attendee()
    status, data = await edit_attendees(sent={"id": id_to_edit, "date_arrived": "2023-12-15 10:00:00"})
    assert status == 200
    ...
    assert response.status_code == 200
    data = response.json()["data"]
    assert data[0]["arrived"] == True and data[0]["date_arrived"] == "2023-12-15 10:00:00"
...
async def test_edit_attendees_database_not_found():
    status, data = await edit_attendees(sent={"id": 999999, "full_name": "Name Not Found"})
    assert status == 400
    assert data["error_code"] == 101
