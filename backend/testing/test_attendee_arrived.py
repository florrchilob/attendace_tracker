...
async def test_attendee_arrived_missing_identifiers():
    response = await attendee_arrived(sent={})
    assert response["status"] == 400
    assert response["data"]["error_code"] == 101

@pytest.mark.asyncio
async def test_attendee_arrived_invalid_mispar_ishi():
    response = await attendee_arrived(sent={"mispar_ishi": "123", "tehudat_zehut": "123456789"})
    assert response["status"] == 400
...
async def test_attendee_arrived_invalid_tehudat_zehut():
    response = await attendee_arrived(sent={"mispar_ishi": "1234567", "tehudat_zehut": "12345678"})
    assert response["status"] == 400
...
async def test_attendee_arrived_nonexistent_attendee(random_data_attendee):
    response = random_data_attendee
    response = await attendee_arrived(sent={"mispar_ishi": response["random_mispar_ishi"], "tehudat_zehut": response["random_tehudat_zehut"]})
    assert response["status"] == 400
...
async def test_attendee_arrived_valid_attendee():
    ...
    response = await attendee_arrived(sent={"mispar_ishi": random_mispar_ishi, "tehudat_zehut": random_tehudat_zehut})
